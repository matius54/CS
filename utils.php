<?php
    //una coleccion de clases que serviran para muchas cosas diferentes
    class SECURITY {
        private static function hexChar() : string {
            return dechex(mt_rand(0,15));
        }
        public static function randomHexStr($length = 32) : string {
            $arr = [];
            for ($i = 0; $i < $length; $i++) {
                $arr[$i] = self::hexChar();
            }
        ...
        public static function password(string $password,string &$hash,string|null &$salt) : string {
